...
_EE_NOMINAL_P = np.array([0.30, 0.0, 1.0])          # from sim_env.reset() line 65
_R_TARGET     = rpy2r(np.deg2rad([90, -0., 90]))     # from sim_env.reset() line 66
...
class PatchedEnv(SimpleEnv2):
    ...
    The only public additions are:
        ...
        get_depth(camera_name) -> np.ndarray
        check_success() -> bool  (inherited from SimpleEnv2)
    """
    ...
    def get_depth(self, camera_name: str = 'agentview') -> np.ndarray:
        """
        Return a metric depth map (in metres) for the given camera.

        Must be called after grab_image() in the same step — both use
        self.env.cams[cam_idx] which requires the viewer context to be active.

        NDC→metric conversion (MuJoCo):
            depth = near / (1 - depth_raw * (1 - near/far))

        Pixels at the far clipping plane are set to np.nan.

        Args:
            camera_name: Name of the MuJoCo camera (default 'agentview').

        Returns:
            depth: (H, W) float32 array of metric depth values in metres.
        """
        cam_idx = self.env.model.camera(camera_name).id
        depth_raw = self.env.cams[cam_idx].depth.copy().astype(np.float32)

        near = self.env.model.vis.map.znear * self.env.model.stat.extent
        far  = self.env.model.vis.map.zfar  * self.env.model.stat.extent

        # NDC → metric
        denominator = 1.0 - depth_raw * (1.0 - near / far)
        depth = np.where(denominator > 0, near / denominator, np.nan).astype(np.float32)

        # Mark far-plane pixels as nan
        depth[depth_raw >= 1.0] = np.nan
